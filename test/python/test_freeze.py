...
class TestFreeze(unittest.TestCase):
    def setUp(self):
        ...
        if not mpath.exists():
            ...
            Path(mpath).joinpath('opt', 'empty_dir').mkdir(parents=True)
    ...
    def test_something(self):
        ...
        for n in 'create_date source'.split():
            del res[n]
        c = {
            'empty_dirs': [{'mode': 16877, 'modestr': 'drwxr-xr-x', 'rel': 'opt/empty_dir'}],
            'files': [{'mode': 33188,
                       'modestr': '-rw-r--r--',
            ...
            'repo_specs': ({'links': [],
                            'name': 'repo_def',
                            'path': 'view/util/grsync/target/mock/view/repo_def',
                            'remotes': [{'is_master': True,
                                         'name': 'origin',
                                         'url': 'https://github.com/plandes/zenbuild'}]},
                           {'links': [{'source': 'view/util/grsync/target/mock/symlink_to_repo',
                                       'target': 'view/util/grsync/target/mock/view/repo_src/README.md'}],
                            'name': 'repo_src',
                            'path': 'view/util/grsync/target/mock/view/repo_src',
                            'remotes': [{'is_master': True,
                                         'name': 'origin',
                                         'url': 'https://github.com/plandes/zenbuild'}]}),}
